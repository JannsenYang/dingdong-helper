...
非常重要 非常重要 非常重要 405问题解决方案（手机APP和小程序真机操作也无法进入购物车或无法下单），不保证完全有效,退出App账号重新登录，尝试刷新购物车和提交订单是否正常，如果正常则退出小程序重新登录后再抓包，替换UserConfig中的cookie和device_token。

## issues中今天有不少不能下单的，根据我自身和朋友的反馈并没有出现，推荐大家还是用时间触发5点59和8点29执行2分钟，没抢到也不要再执行了，对于被风控的兄弟说声抱歉，如果按照我文档中的几种方式解决不了只能换号了过一段时间再尝试被封的号

## 最新发现 最新发现 最新发现 

购物车看不见商品一直提示当前人多拥挤，如果不是这个提示请参考注意事项中的第5项 。先给结论，不是封号，是叮咚服务器上你个人的购物车数据出现异常了，本人亲自经历正常人工App添加购物车，加之前购物车是好的，加着加着就进不去了购物车了，一直提示当前人多拥挤，通过一系列骚操作给救回来了。

1. 进入购物车，注意看右上角有个删除按钮，如果是灰色的，下面首页分类购物车来回点再点点购物车里的我常买，一直来回切，直到切到右上角删除按钮变成绿色
2. 点击删除按钮，再点击左下角全选，如果不出意外购物车里的东西就出来了
3. 点击清空失效商品，再切换其他页面再进购物车
4. 如果还没好继续重复1、2、3步骤，下次又会刷出来失效商品，一直清理清到没有失效商品就好了
...
5. 405问题解决方案（手机APP和小程序真机操作也无法下单），不保证完全有效,退出App账号重新登录，尝试刷新购物车和提交订单是否正常，如果正常则退出小程序重新登录后再抓包，替换UserConfig中的cookie和device_token。
6. 接口如果出现405状态有以3种可能 1.偶发，无需处理 2.不要长时间运行程序，参考上面的第2点  3.一个账号下单数有时会有限制 参考上面的第3点
7. 根据反馈有少部分人的站点是假库存，可能是怕大家闹事，开放购买之前能看到购物车里有，但是根本就不可能买到，第一秒下单瞬间很多东西就没了，我也是，几百块的购物车最后下单几十块，我同时用app人工操作了购物车确实是没货了，不是程序问题。
8. 日期20220412我这个站点是6点开始陆续上东西，6点之前购物车没有东西可以买
