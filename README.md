...
完全还原签名算法，保证和真机一模一样的请求参数，之前请求中有签名字段，但是后端无验证，为了避免留下把柄，展现了一把我深厚的js功底，现在请求和小程序一模一样，没有任何参数不同
...
2. 一天不要下超过两单
3. 慎用哨兵模式，执行时间不宜超过3小时，并且尽量加大请求和轮询间隔

尝试方案
1. 过一段时间再试
2. 重新登录
3. 拨打客服电话（按以往其他平台的经验，底气要十足，就说不知道）
4. 换号
...
### 2022.04.15
1. 完全还原签名算法，保证和真机一模一样的请求参数，之前请求中有签名字段，但是后端无验证，为了避免留下把柄，展现了一把我深厚的js功底，现在请求和小程序一模一样，没有任何参数不同
