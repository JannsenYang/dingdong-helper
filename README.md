...
5. 优惠券不知道为什么大概率是不生效的，因为过了高峰期配送资源都没了，不让下单所以不好调试，忌讳不能使用优惠券者勿用，如果有人能修复此问题请提pr，万分感谢谢
...
8. 如果想非高峰期捡漏，修改Application中的并发数量、等待时间，并发改成1（不并发，和人操作一致），等待时间可以选择10秒（比人还慢）以上，此并发量可长期运行，但如果是配送额度满了此方法可能不会生效，配送额度大概率只有6点和8点30更新一次，另需要注释掉Api中202行Application.map.put("end", new HashMap<>()); 这一行是判断购物车无商品是否停掉线程的
