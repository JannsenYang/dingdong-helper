...
非常重要 非常重要 非常重要 405问题解决方案（手机APP和小程序真机操作也无法下单），不保证完全有效,退出App账号重新登录，尝试刷新购物车和提交订单是否正常，如果正常退出小程序重新登录后再抓包，替换UserConfig中的cookie和device_token。
...
5. 405问题解决方案（手机APP和小程序真机操作也无法下单），不保证完全有效,退出App账号重新登录，尝试刷新购物车和提交订单是否正常，如果正常退出小程序重新登录后再抓包，替换UserConfig中的cookie和device_token。
